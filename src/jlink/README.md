...
- **`udevadm` shim during install.** Segger's postinst calls `udevadm control --reload-rules`. Slim Debian bases (`python:3.x-trixie` and similar) ship without the `udev` package, so `udevadm: not found` aborts dpkg. Even on bases that *do* ship `udev`, there is no udev daemon running inside an image build, so the reload returns non-zero and the postinst exits with `Failed to update udevadm rules.` either way. The install script writes a no-op `/usr/sbin/udevadm` for the duration of the `apt-get install ./jlink.deb` step and removes it afterwards. Udev rules are a host concern — the rules file Segger ships is placed at `/etc/udev/rules.d/99-jlink.rules` regardless, and applies when the host (not the container) reloads udev.
